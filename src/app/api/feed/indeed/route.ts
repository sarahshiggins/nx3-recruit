...
function wrapCdata(html: string): string {
  // Prevent unescaped CDATA terminator from breaking the section.
...
function markdownToHtml(md: string): string {
  // Lightweight markdown → HTML converter for description bodies. Indeed
  // expects HTML inside <description>; CDATA-wrapping handles entities.
...
function buildJobXml(job: JobListing, postedAt: Date): string {
  ...
  return `  <job>
    ...
    <date>${escapeXml(rfc2822(postedAt))}</date>
...
<source>
  <publisher>Nexus3</publisher>
  <publisherurl>https://nexus3.ai</publisherurl>
